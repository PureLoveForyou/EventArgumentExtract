...
from src.genie.RAMS_data_module import RAMSDataModule
from src.genie.ACE_data_module import ACEDataModule
from src.genie.KAIROS_data_module import KAIROSDataModule 
from src.genie.Military_data_module import MilitaryDataModule
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=['RAMS', 'ACE', 'KAIROS', 'MilitaryExercise']
    )
    ...
    elif args.dataset == 'MilitaryExercise':
        dm = MilitaryDataModule(args)
    ...
    trainer = Trainer(
        ...
        # val_check_interval=0.4, # use float to check every n epochs 
    ...
    if args.eval_only: 
        ...
        print('Test cost {time} s'.format(time=(datetime.datetime.now() - time0)))
    else:
        dm.setup('fit')
