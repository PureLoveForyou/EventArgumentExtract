...
class MilitaryDataModule(pl.LightningDataModule):
    ...
    def create_gold_gen(self, ex, ontology_dict,mark_trigger=True):
        ...
        input_template = re.sub(r'<arg\d[\d]?>', '<arg>', template) 
        space_tokenized_input_template = input_template.split(' ')
        # print('分词；', space_tokenized_input_template)
        ...
        output_template = re.sub(r'<arg\d[\d]?>','<arg>', template ) 
            ...
            for split,f in [('train',self.hparams.train_file), ('val',self.hparams.val_file), ('test',self.hparams.test_file)]:
                with open(f,'r') as reader,  open('preprocessed_data/{}.jsonl'.format(split), 'w') as writer:
                    for lidx, line in enumerate(reader):
                        ...
                        # print('输入模板', input_template)
                        # print('输出模板', output_template)
                        # print('context:', context)
                        # with open('/root/autodl-tmp/gen-arg-Chinese/tmp.txt', 'w', encoding='utf-8') as out:
                        #     out.write('输入模板' +  str(input_template) + '\n')
                        #     out.write('输出模板' +  str(output_template) + '\n')   
                        #     out.write('context:' + str(context))
